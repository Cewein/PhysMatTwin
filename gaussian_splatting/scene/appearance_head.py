...
class AppearanceHeadWrapper:
    """
    Wrapper class that manages the appearance head and its optimizer,
    making it easy to integrate with the existing GaussianModel training.
    """
    ...
    def __init__(
        ...
        self.device = device
        self.model = NeuralAppearanceHead(
            sh_degree=sh_degree,
        ...
        # Stiffness normalization bounds (can be set externally)
        self.stiffness_min = None
        self.stiffness_max = None

    def set_stiffness_bounds(self, min_val: float, max_val: float):
        """Set the bounds for stiffness normalization."""
        self.stiffness_min = min_val
        self.stiffness_max = max_val
    ...
    def compute_colors(
        ...
        """
        Compute RGB colors using the appearance head.

        Args:
            sh_features: SH coefficients from GaussianModel.get_features
            view_dirs: Normalized view directions
            stiffness: Raw stiffness values (will be normalized internally)

        Returns:
            RGB colors (N, 3)
        """
        # Normalize stiffness if provided
        if stiffness is not None:
            stiffness = normalize_stiffness(
                stiffness,
                self.stiffness_min,
                self.stiffness_max
            )

        return self.model(sh_features, view_dirs, stiffness)
    ...
    def load(self, path: str):
        ...
        self.stiffness_min = checkpoint.get('stiffness_min')
        self.stiffness_max = checkpoint.get('stiffness_max')
